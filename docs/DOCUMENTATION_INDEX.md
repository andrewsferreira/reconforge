...
**Status:** ✅ Complete (332 lines, updated 2026-07-14 for `mcp.yaml`)

`tools.yaml` schema (binary paths, timeouts, scan profiles, safety settings), `profiles.yaml` schema (OPSEC modes, timing, technique toggles, noise-level gates), `mcp.yaml` schema (the `reconforge/mcp/` server's `mcp.allow_intrusive_execution` gate), `ConfigLoader` API, `ToolConfig` typed accessor, and `ProfileLoader` resolution logic. Single source of truth — no env-var overrides.
...
**Status:** ✅ Complete — all 15 planned phases done or assessed, plus a post-hoc security-architecture rewrite (SEC-P0, 2026-07-15) that replaced the original `explicit_confirmation`-field approval model with genuine out-of-band operator approval (17 tools: 12 read-only + 5 execution-related, plus structured error codes, the server-wide config gate, the user-facing setup guide, structured audit events, a README section + 3 runnable examples including a self-contained safe demonstration, 7 read-only resources, a broader test-category audit that found and fixed a real web/api target-validation bug, and the SEC-P0 approval-architecture rewrite); Phase 12 assessed with no gap found; CI recovery + quality-gate scope widening in v2.11.1 (updated 2026-07-15)

The design for `reconforge/mcp/`, the package that lets Claude Desktop/Claude Code act as an MCP *client* against a ReconForge-hosted MCP *server* (the reverse relationship from `core/adapters/burp/`, where ReconForge is the MCP client). Covers the trust-boundary diagram, the `trusted_metadata`/`untrusted_evidence` response-shape split, the `SAFE_READ_ONLY → PROHIBITED` execution-tier policy table, the human-approval model, representative request/response schemas, and which existing primitives each planned tool reuses rather than duplicates. `reconforge/mcp/server.py` (Phase 2) builds a real `mcp` SDK `Server`, runnable via `reconforge mcp serve`. `reconforge/mcp/tools.py` + `services.py` + `schemas.py` (Phase 3) register all 12 read-only tools. `reconforge/mcp/sanitization.py` (Phase 4) centralizes untrusted-content handling; every response carries a `trust: "server_generated"` marker; a 26-payload adversarial test suite proves injection payloads stay inert. `reconforge/mcp/policy.py` (Phase 5) implements the tier taxonomy. A real stdio-corruption bug (`ReconLogger` logging to `sys.stdout` unconditionally, breaking the JSON-RPC stream) was found via a genuine subprocess test — not the in-memory transport every other test in this package uses — and fixed in `server.py`. `tools.py` (Phase 8) now surfaces each error's `code` and, for policy denials, `missing_requirements` as `structuredContent` on the MCP error response, instead of only the SDK's generic plain-text fallback. `config/mcp.yaml` (Phase 9) adds `mcp.allow_intrusive_execution` (default `false`), a server-wide off switch for INTRUSIVE-tier phases layered on top of `evaluate()`'s existing per-request requirements — not settable via the MCP request itself. `reconforge/mcp/audit.py` (Phase 13) emits one sanitized JSON line to stderr per tool call, success or failure, wired into `tools.py`'s single dispatch choke point. README.md's "Claude and MCP Integration" section and `examples/claude_mcp/*.py` (Phase 14) give a quick-start path outside this design doc. `reconforge/mcp/jobs.py` (Phase 6) adds `reconforge_start_execution`/`reconforge_get_execution_status` for phases that might run long; deliberately no `cancel` tool, since `core/runner.py` has no cooperative-cancellation hook. `reconforge/mcp/resources.py` (Phase 7) registers `list_resources`/`read_resource` handlers exposing 7 resources — 6 curated docs plus a live module catalog — under a hardcoded `reconforge://` URI allowlist, distinct from the tool-call interface; `audit.py::emit_resource_read_audit_event()` gives it the same audit-logging parity as tool calls. **SEC-P0 (2026-07-15)** replaced `explicit_confirmation: bool` — a request field Claude itself could set — with a real two-process approval flow: `reconforge_request_execution` (new) creates a pending, disk-backed `ApprovalRequest`; only a human operator running `reconforge mcp approvals approve <request_id>` in a separate terminal (`reconforge/mcp/approvals.py` + `approvals_cli.py`, no MCP tool or resource equivalent) can approve it; `reconforge_execute_approved_phase`/`reconforge_start_execution` now take *only* a `request_id` and atomically consume the approval exactly once, verified against a canonical hash of the exact operation approved. `reconforge_get_approval_status` (new) polls status without exposing any secret.

---

### CLAUDE_MCP_INTEGRATION.md
📍 **Location:** [`docs/CLAUDE_MCP_INTEGRATION.md`](CLAUDE_MCP_INTEGRATION.md)
**Status:** ✅ Complete (Phase 10, 2026-07-14; security model updated for SEC-P0's out-of-band approval architecture, 2026-07-15)

The user-facing setup guide, distinct from the implementation plan above (design rationale) — this one is purely "how do I connect a client." Covers the `mcp` extra install (`pip install -e ".[mcp]"`), Claude Desktop's `claude_desktop_config.json` `mcpServers` block, Claude Code's `claude mcp add`, a condensed security-model summary, a table of all 17 tools plus the 7 read-only resources, a read-only exploration walkthrough, and a step-by-step walkthrough for authorizing real execution (scope YAML → `reconforge workflow --engagement` → `reconforge_request_execution` → operator runs `reconforge mcp approvals approve` out-of-band → `reconforge_execute_approved_phase`), plus the four most user-relevant known limitations (no credentialed execution, free-form scope/output-base path parameters, one execution at a time per server process, no execution cancellation).
