...
> Status: Phase 1 (this document) — done. Phase 2 (MCP Server Foundation) — done. **Phase 3 (Read-Only MCP Capabilities), part 1/2 — done**: 8 of the 12 planned read-only tools are implemented and registered — `reconforge_get_status`, `reconforge_list_modules`, `reconforge_get_module_details`, `reconforge_list_engagements`, `reconforge_get_engagement`, `reconforge_get_scope`, `reconforge_plan_workflow`, `reconforge_dry_run`. Each wraps a real existing primitive rather than fabricating data: module introspection reads each module class's own `MODULE_NAME`/`VALID_PHASES`; `reconforge_dry_run` instantiates the real module classes with `dry_run=True` and reads back `Runner.get_command_log()` (already secret-redacted); `reconforge_get_scope`/`reconforge_plan_workflow` reuse `ScopeAuthorization.from_file()` unchanged; engagement/scope data is read straight off the same on-disk layout the CLI itself writes. The 4 findings/reporting tools (`reconforge_get_findings`, `reconforge_get_finding`, `reconforge_summarize_findings`, `reconforge_generate_report`) are next, pending operator go-ahead. See `docs/CLAUDE_MCP_INTEGRATION.md` (not created yet — added once all read-only tools exist) for the eventual user-facing setup guide.
