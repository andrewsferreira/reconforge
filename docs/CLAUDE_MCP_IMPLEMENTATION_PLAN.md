...
> Status: Phase 1 — done. Phase 2 (MCP Server Foundation) — done. Phase 3 (Read-Only MCP Capabilities) — done: all 12 read-only tools implemented (status/module/engagement/scope inspection, workflow planning, dry-run, findings/reporting), each wrapping a real existing primitive rather than fabricating data. Phase 4 (Prompt-Injection Resistance) — done: `reconforge/mcp/sanitization.py` centralizes untrusted-content handling (NFC normalization, ANSI/control-char stripping, secret redaction, truncation), every response carries an explicit `trust: "server_generated"` marker, a 26-payload adversarial test suite proves injection payloads stay inert. **Phase 5 (Controlled Execution), part 1/2 — done**: `reconforge/mcp/policy.py` implements the `SAFE_READ_ONLY → PROHIBITED` tier taxonomy — `classify_phase(module, phase)` maps every real `(module, phase)` pair (plus network's `brute_force` parameter) to a tier, cross-referenced against the actual opt-in gates found in the module code (not invented), and `evaluate()` checks a tier's requirements against caller-supplied facts, engineered so it can never manufacture its own approval — `explicit_confirmation`/`approval_id` are inputs it only ever checks, never sets. `reconforge_plan_workflow`'s response now surfaces each phase's tier (`phase_tiers`) and lists which phases need INTRUSIVE/CREDENTIAL_USE-level approval (`required_approvals`), so the classification is genuinely visible to Claude before Phase 5 part 2 exists — **no `reconforge_execute_approved_phase` tool exists yet; nothing this policy module classifies can currently be executed through MCP.** Phase 5 part 2 (the actual execution tool, its 17-point verification, and the job/cancellation model) is next, pending operator go-ahead. See `docs/CLAUDE_MCP_INTEGRATION.md` (not created yet) for the eventual user-facing setup guide.
