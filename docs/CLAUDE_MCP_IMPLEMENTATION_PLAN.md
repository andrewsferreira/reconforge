...
> Status: Phase 1 (this document) — done. Phase 2 (MCP Server Foundation) — done. Phase 3 (Read-Only MCP Capabilities) — done: all 12 planned read-only tools implemented and registered (`reconforge_get_status/list_modules/get_module_details/list_engagements/get_engagement/get_scope/plan_workflow/dry_run/get_findings/get_finding/summarize_findings/generate_report`), each wrapping a real existing primitive (`Runner(dry_run=True)`, `ScopeAuthorization`, `EngagementManager`, `FindingsManager`'s on-disk `findings.json` layout) rather than fabricating data. **Phase 4 (Prompt-Injection Resistance) — done**: `reconforge/mcp/sanitization.py` centralizes untrusted-content handling (`sanitize_untrusted_text()`: NFC Unicode normalization, ANSI/terminal-escape stripping, control-character stripping, `core/logger.py::sanitize_log()` secret redaction, line/byte-length truncation with an explicit `truncated` flag; `is_binary_content()` for a future raw-output/HTTP-body path, not reachable from any tool yet). Every response schema now inherits a `TrustedResponse` base carrying an explicit `trust: "server_generated"` root marker (belt-and-suspenders on top of the `trusted_metadata`/`untrusted_evidence` split). A 26-payload adversarial test suite (instruction-override text, fake "call the execute tool" directives, fake scope-expansion/env-var-reveal directives, HTML comments with fake system prompts, terminal escape sequences, nested JSON containing a fake tool-call structure, oversized responses, base64/URL-encoded payloads) proves every payload survives as inert string data under `untrusted_evidence` — never parsed, never promoted to a trusted field, never causing a different code path to run. Phase 5 (Controlled Execution) is next, pending operator go-ahead. See `docs/CLAUDE_MCP_INTEGRATION.md` (not created yet — added once controlled execution exists too) for the eventual user-facing setup guide.
...
- `validate_arg()` is called by only 4 of 28 tool wrappers today (`docs/ARCHITECTURE_REVIEW.md`'s tracked P2 item) — MCP-triggered
  execution inherits this pre-existing gap; it is not made worse by MCP, but it is not fixed by MCP either. `module_parameters` in
  `ExecuteApprovedPhaseRequest` is deliberately narrow (bounded key/value pairs per module, schema-validated) specifically to avoid
  MCP becoming a new path that reaches an unvalidated wrapper with attacker-influenced input more easily than the CLI already does.
