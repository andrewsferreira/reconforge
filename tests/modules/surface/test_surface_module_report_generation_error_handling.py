"""Phase 21: SurfaceModule._generate_reports() swallowed any exception
during the report-file writes with a bare `except Exception as e:
...
def _make_module(tmp_path) -> SurfaceModule:
    module = SurfaceModule.__new__(SurfaceModule)
    module.MODULE_NAME = "surface"
...
def test_generate_reports_raises_module_error_instead_of_swallowing(tmp_path):
    ...
    assert exc_info.value.module == "surface"
